...
func AttachUprobes(soPath string, pid int, bpfModule *bcc.Module, uprobeList []structs.Uprobe) error {
	for _, probe := range uprobeList {
		...
		case structs.EntryType_Matching_Suf:
		...
		case structs.ReturnType_Matching_Suf_Addr:
		...
		case structs.EntryType_Matching_Pre:
			slog.Debug("Loading matching uprobe", "hook name: ", probe.HookName, "function: ", functionToHook)
		...
		case structs.ReturnType_Matching_Pre:
