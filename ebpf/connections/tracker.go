...
func (conn *Tracker) IsComplete() bool {
	...
	if complete {
		utils.LogProcessing("Connection closed", "fd", conn.connID.Fd, "id", conn.connID.Id, "closeTimestamp", conn.closeTimestamp, "currentTimestamp", uint64(time.Now().UnixNano()))
	}
	return complete
}
...
func (conn *Tracker) AddOpenEvent(event structs.SocketOpenEvent) {
	...
	if conn.openTimestamp != 0 {
		utils.LogIngest("Changing conn open timestamp", "current", conn.openTimestamp, "new", now)
	}
