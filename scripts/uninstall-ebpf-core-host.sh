...
#   sudo EBPF_ROOT=/opt/akto/ebpf ./uninstall-ebpf-core-host.sh -y
#
...
# ebpf-run.sh path is unique per install root
pkill -TERM -f "${EBPF_ROOT}/ebpf-run.sh" 2>/dev/null || true
sleep 1
pkill -KILL -f "${EBPF_ROOT}/ebpf-run.sh" 2>/dev/null || true
